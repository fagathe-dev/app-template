**Rôle**: En tant que développeur front-end Typescript, je veux que tu crées un prompt pour générer une fonction fetchApi en Typescript pour interagir avec une API RESTful, afin de faciliter les appels API dans mon application.
...
const fetchAPI = async <T>(url: string, options: RequestInit = {}): Promise<FetchResponse> => {
  try {
...
- Destination du prompt : `.github/prompts/typescript/fetchApi.prompt.md`.
