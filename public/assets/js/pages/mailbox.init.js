...
function emailDetailShow() {
  ...
  Array.from(document.querySelectorAll('.message-list a')).forEach(function (t) {
    ...
    Array.from(document.querySelectorAll('.close-btn-email')).forEach(function (t) {
  ...
  Array.from(document.querySelectorAll('.email-menu-btn')).forEach(function (e) {
    ...
    window.addEventListener('click', function (e) {
      document.querySelector('.email-menu-sidebar').classList.contains('menubar-show') &&
        (t || document.querySelector('.email-menu-sidebar').classList.remove('menubar-show'), (t = !1));
    }),
    favouriteBtn();
}
function checkBoxAll() {
  ...
  function t() {
    ...
    Array.from(e).forEach(function (e) {
      ...
      (document.getElementById('email-topbar-actions').style.display = t > 0 ? 'none' : 'block'),
      t > 0
        ? Array.from(e).forEach(function (e) {
  ...
  function a() {
    var e = document.querySelectorAll('.tab-pane.show .checkbox-wrapper-mail input'),
      a = document.querySelectorAll('.tab-pane.show .checkbox-wrapper-mail input:checked').length;
    Array.from(e).forEach(function (e) {
      (e.checked = !1), e.parentNode.parentNode.parentNode.classList.remove('active');
    }),
      (document.getElementById('email-topbar-actions').style.display = a > 0 ? 'none' : 'block'),
      a > 0
        ? Array.from(e).forEach(function (e) {
            (e.checked = !1), e.parentNode.parentNode.parentNode.classList.remove('active');
          })
  ...
  Array.from(e).forEach(function (e) {
    e.addEventListener('click', function (e) {
      ...
      (a.checked = l > 0),
        ...
        (document.getElementById('email-topbar-actions').style.display = l > 0 ? 'block' : 'none');
    });
  }),
  ...
  Array.from(document.querySelectorAll('.mail-list a')).forEach(function (e) {
    e.addEventListener('click', function () {
      ...
      if (
        ...
        document.getElementById('mail-filter-navlist').style.display = 'All' != a && 'Inbox' != a ? 'none' : 'block';
      }
...
function emailDetailChange() {
  Array.from(document.querySelectorAll('.message-list li')).forEach(function (e) {
    e.addEventListener('click', function () {
      var t = e.querySelector('.checkbox-wrapper-mail .form-check-input').value;
      document.querySelector('.remove-mail').setAttribute('data-remove-id', t);
      ...
      Array.from(document.querySelectorAll('.accordion-item.left')).forEach(function (t) {
      ...
      var i = document.querySelector('.user-name-text').innerHTML,
        c = document.querySelector('.header-profile-user').getAttribute('src');
      Array.from(document.querySelectorAll('.accordion-item.right')).forEach(function (e) {
        (e.querySelector('.email-user-name-right').innerHTML = i), e.querySelector('img').setAttribute('src', c);
      });
    });
  });
}
document.querySelectorAll('.email-chat-list a').forEach(function (e) {
  if (e.classList.contains('active')) {
    ...
    var t = document.querySelector('.email-chat-list a.active').querySelector('.chatlist-user-name').innerHTML,
      a = document
  ...
  e.addEventListener('click', function (t) {
    ...
    document.querySelector('.email-chat-detail .profile-username').innerHTML = l;
    var c = document.getElementById('users-conversation');
    Array.from(c.querySelectorAll('.left .chat-avatar')).forEach(function (e) {
      i ? e.querySelector('img').setAttribute('src', i) : e.querySelector('img').setAttribute('src', dummyUserImage);
    });
  });
}),
...
function resizeEvent() {
  if (document.documentElement.clientWidth < 767) {
    ...
    e && e.classList.remove('active'), (document.getElementById('emailchat-detailElem').style.display = 'none');
  }
}
