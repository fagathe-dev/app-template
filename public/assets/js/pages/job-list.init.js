function getChartColorsArray(e) {
  if (null !== document.getElementById(e)) {
    ...
    return (t = JSON.parse(t)).map(function (e) {
      ...
      var o = e.split(',');
      if (2 == o.length) {
        var a = getComputedStyle(document.documentElement).getPropertyValue(o[0]);
        return (a = 'rgba(' + a + ',' + o[1] + ')');
      }
      return t;
    });
...
function loadJobListData(e, t) {
  ...
  t < 1 && (t = 1), t > n && (t = n), (document.querySelector('#job-list').innerHTML = '');
  for (var o = (t - 1) * itemsPerPage; o < t * itemsPerPage && o < e.length; o++) {
    if (e[o] && e[o].tags) {
      var a = e[o].tags,
        i = '';
      Array.from(a).forEach((e, t) => {
        i += '<span class="badge bg-primary-subtle text-primary me-1">' + e + '</span>';
      });
    }
    e[o] &&
...
function selectedPage() {
  for (var e = document.getElementById('page-num').getElementsByClassName('clickPageNumber'), t = 0; t < e.length; t++)
    t == currentPage - 1 ? e[t].parentNode.classList.add('active') : e[t].parentNode.classList.remove('active');
}
function paginationEvents() {
  ...
  prevButton.addEventListener('click', function () {
    currentPage > 1 && (currentPage--, loadJobListData(allJobList, currentPage));
  }),
    nextButton.addEventListener('click', function () {
      currentPage < e() && (currentPage++, loadJobListData(allJobList, currentPage));
    }),
    (function () {
      ...
      for (var n = 1; n < e() + 1; n++)
        t.innerHTML +=
          "<div class='page-item'><a class='page-link clickPageNumber' href='javascript:void(0);'>" + n + '</a></div>';
    })(),
    document.addEventListener('click', function (e) {
      'A' == e.target.nodeName &&
        e.target.classList.contains('clickPageNumber') &&
        ((currentPage = e.target.textContent), loadJobListData(allJobList, currentPage));
    }),
    selectedPage();
}
getJSON('job-list.json', function (e, t) {
  null !== e
    ...
    : (loadJobListData((allJobList = t), currentPage), paginationEvents(), sortElementsById());
});
...
function jobDetailShow() {
  Array.from(document.querySelectorAll('#job-list .joblist-card')).forEach(function (e) {
    e.querySelector('.viewjob-list').addEventListener('click', function () {
      ...
      (document.querySelector('#cover-img').src = t),
        ...
        (document.querySelector('#job-overview .view-companyname').innerHTML = a),
        (document.querySelector('#job-overview .view-location').innerHTML = l),
        (document.querySelector('#job-overview .view-desc').innerHTML = i),
        (document.querySelector('#job-overview .view-type').innerHTML = r),
        (document.querySelector('#job-overview .view-postdate').innerHTML = c),
        (document.querySelector('#job-overview .view-experience').innerHTML = s);
    });
  });
}
      ...
      e.addEventListener(
        ...
        function (n) {
          if (e.checkValidity()) {
            ...
            loadJobListData(allJobList, currentPage), document.getElementById('close-jobListModal').click();
