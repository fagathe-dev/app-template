var str_dt = function (e) {
    ...
    return (
      ...
      [day + ' ' + month + ',' + year + " <small class='text-muted'>" + n + ':' + l + ' ' + s + '</small>']
    );
  },
  ...
  checkAll = ((choices = new Choices(isPaymentEl, { searchEnabled: !1 })), document.getElementById('checkAll'));
checkAll &&
  (checkAll.onclick = function () {
    ...
    document.getElementById('remove-actions').style.display = t > 0 ? 'none' : 'block';
  });
var perPage = 8,
  editlist = !1,
  options = {
...
function SearchData() {
  ...
  orderList.filter(function (s) {
    ...
    return (
      (i = 'all' == r || 'all' == e || r == e),
      (d = 'all' == s.values().payment || 'all' == t || s.values().payment == t),
      (o =
        new Date(s.values().date.slice(0, 12)) >= new Date(n) && new Date(s.values().date.slice(0, 12)) <= new Date(l)),
      i && d && o ? i && d && o : i && d && '' == a ? i && d : d && o && '' == a ? d && o : void 0
    );
  }),
    orderList.update();
}
  ...
  Array.from(document.getElementsByName('checkAll')).forEach(function (e) {
    e.addEventListener('change', function (t) {
      ...
      t.target.closest('tr').classList.contains('table-active'),
        (document.getElementById('remove-actions').style.display = a > 0 ? 'block' : 'none');
    });
  });
}
...
function deleteMultiple() {
  ...
  for (i = 0; i < e.length; i++)
    if (1 == e[i].checked) {
      var t = e[i].parentNode.parentNode.parentNode.querySelector('td a').innerHTML;
      ids_array.push(t);
    }
    ...
    ? Swal.fire({
        ...
        if (e.value) {
          for (i = 0; i < ids_array.length; i++)
            orderList.remove(
              'id',
              '<a href="apps-ecommerce-order-details.html" class="fw-medium link-primary">' + ids_array[i] + '</a>'
            );
