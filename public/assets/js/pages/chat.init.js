function searchMessages() {
  ...
  (e = document.getElementById('searchMessage')),
    ...
    (s = document.getElementById('users-conversation').getElementsByTagName('li')),
    Array.from(s).forEach(function (e) {
      (n = e.getElementsByTagName('p')[0] ? e.getElementsByTagName('p')[0] : ''),
        (n.textContent || n.innerText ? n.textContent || n.innerText : '').toUpperCase().indexOf(t) > -1
          ? (e.style.display = '')
...
!(function () {
  ...
  function n() {
    ...
    Array.from(document.querySelectorAll('.chat-user-list li a')).forEach(function (t) {
      t.addEventListener('click', function (t) {
        e.forEach(function (e) {
          e.classList.add('user-chat-show');
        });
        var s = document.querySelector('.chat-user-list li.active');
        s && s.classList.remove('active'), this.parentNode.classList.add('active');
      });
    }),
    ...
    (t = e),
      (n = function (e, t) {
        if (null !== e) console.log('Something went wrong: ' + e);
          ...
          n.forEach(function (e, t) {
            if (a > 0) a -= 1;
              ...
              1 != e.from_id && (l += '<div class="chat-avatar"><img src="' + i.profile + '" alt=""></div>'),
                (l += '<div class="user-chat-content">'),
                (l += d(e.id, e.msg, e.has_images, e.has_files, e.has_dropDown)),
                n[t + 1] &&
        ...
        x.querySelectorAll('.delete-item').forEach(function (e) {
          ...
          b(),
          (function () {
            var e = x.querySelectorAll('.reply-message'),
            ...
            e.forEach(function (e) {
              e.addEventListener('click', function () {
                ...
                var a = e.closest('.ctext-wrap').children[0].children[0].innerText;
                document.querySelector('.replyCard .replymessage-block .flex-grow-1 .mb-0').innerText = a;
                var r = document.querySelector('.user-chat-topbar .text-truncate .username').innerHTML,
                  i = e.closest('.chat-list') ? (e.closest('.chat-list').classList.contains('left') ? r : 'You') : r;
                document.querySelector('.replyCard .replymessage-block .flex-grow-1 .conversation-name').innerText = i;
              });
            });
          })(),
          (function () {
            var e = w.querySelectorAll('.reply-message'),
              t = document.querySelector('.replyCard'),
              n = document.querySelector('#close_toggle');
            e.forEach(function (e) {
              e.addEventListener('click', function () {
                ...
                var a = e.closest('.ctext-wrap').children[0].children[0].innerText;
                document.querySelector('.replyCard .replymessage-block .flex-grow-1 .mb-0').innerText = a;
                var r = e.closest('.user-chat-content').querySelector('.conversation-name .name').innerText,
                  i = e.closest('.chat-list') ? (e.closest('.chat-list').classList.contains('left') ? r : 'You') : r;
                document.querySelector('.replyCard .replymessage-block .flex-grow-1 .conversation-name').innerText = i;
              });
            });
          ...
          (function () {
            var e = x.querySelectorAll('.copy-message');
            e.forEach(function (e) {
              e.addEventListener('click', function () {
          ...
          (function () {
            var e = w.querySelectorAll('.copy-message');
            e.forEach(function (e) {
              e.addEventListener('click', function () {
                var t = e.closest('.ctext-wrap').children[0]
                  ? e.closest('.ctext-wrap').children[0].children[0].innerText
      ...
      (a.onload = function () {
        var e = a.status;
        200 === e ? ((document.getElementById('elmLoader').innerHTML = ''), n(null, a.response)) : n(e, a.response);
      }),
      a.send();
  }
  function p(e) {
    setTimeout(function () {
      var t = document.getElementById(e).querySelector('#chat-conversation .simplebar-content-wrapper')
          ? document.getElementById(e).querySelector('#chat-conversation .simplebar-content-wrapper')
          : '',
        s = document.getElementsByClassName('chat-conversation-list')[0]
          ? document.getElementById(e).getElementsByClassName('chat-conversation-list')[0].scrollHeight -
            window.innerHeight +
            335
          : 0;
  ...
  c('chat-users-list.json', function (i, l) {
    null !== i
    ...
    n(),
      document.querySelectorAll('#userList li').forEach(function (t) {
        t.addEventListener('click', function () {
          (r = 'users'), m(), (a = 'users-chat');
          var n = t.getAttribute('id'),
            i = t.querySelector('.text-truncate').innerHTML;
          if (
            ((document.querySelector('.user-chat-topbar .text-truncate .username').innerHTML = i),
            (document.querySelector('.profile-offcanvas .username').innerHTML = i),
            1 == s && ((s = !1), document.querySelector('.replyCard').classList.remove('show')),
            document.getElementById(n).querySelector('.userprofile'))
          ) {
            var l = document.getElementById(n).querySelector('.userprofile').getAttribute('src');
            document.querySelector('.user-chat-topbar .avatar-xs').setAttribute('src', l),
              document.querySelector('.profile-offcanvas .avatar-lg').setAttribute('src', l);
          } else document.querySelector('.user-chat-topbar .avatar-xs').setAttribute('src', e), document.querySelector('.profile-offcanvas .avatar-lg').setAttribute('src', e);
          document
            .getElementById('users-conversation')
            .querySelectorAll('.left .chat-avatar')
            .forEach(function (t) {
              l ? t.querySelector('img').setAttribute('src', l) : t.querySelector('img').setAttribute('src', e);
            }),
            window.stop();
        });
      }),
      document.querySelectorAll('#channelList li').forEach(function (e) {
        e.addEventListener('click', function () {
          (a = 'channel-chat'), (r = 'channel'), m();
          var n = e.querySelector('.text-truncate').innerHTML,
            i = document.getElementById('channel-chat');
          (i.querySelector('.user-chat-topbar .text-truncate .username').innerHTML = n),
            (document.querySelector('.profile-offcanvas .username').innerHTML = n),
            i.querySelector('.user-chat-topbar .avatar-xs').setAttribute('src', t),
            document.querySelector('.profile-offcanvas .avatar-lg').setAttribute('src', t),
            1 == s && ((s = !1), document.querySelector('.replyCard').classList.remove('show'));
        });
      });
  }),
    c('chat-contacts-list.json', function (t, a) {
      if (null !== t) console.log('Something went wrong: ' + t);
        ...
        o.forEach(function (e, t) {
          var s = e.profile
            ? '<img src="' + e.profile + '" class="img-fluid rounded-circle" alt="">'
            : '<span class="avatar-title rounded-circle bg-primary fs-10">' + e.nickname + '</span>';
          i =
          ...
          l != e.name.charAt(0) && (document.getElementsByClassName('sort-contact')[0].innerHTML += n),
            (document.getElementById('contact-sort-' + e.name.charAt(0)).innerHTML =
              document.getElementById('contact-sort-' + e.name.charAt(0)).innerHTML + i),
            (l = e.name.charAt(0));
        });
      }
      document.querySelectorAll('.sort-contact ul li').forEach(function (t) {
        t.addEventListener('click', function (n) {
          ...
          if (
            ((document.querySelector('.user-chat-topbar .text-truncate .username').innerHTML = a),
            (document.querySelector('.profile-offcanvas .username').innerHTML = a),
            1 == s && ((s = !1), document.querySelector('.replyCard').classList.remove('show')),
            t.querySelector('.align-items-center').querySelector('.avatar-xxs img'))
          ) {
            var i = t
              .querySelector('.align-items-center')
              .querySelector('.avatar-xxs .rounded-circle')
              .getAttribute('src');
            document.querySelector('.user-own-img .avatar-xs').setAttribute('src', i),
              document.querySelector('.profile-offcanvas .profile-img').setAttribute('src', i);
          } else document.querySelector('.user-own-img .avatar-xs').setAttribute('src', e), document.querySelector('.profile-offcanvas .profile-img').setAttribute('src', e);
          document
            .getElementById('users-conversation')
            .querySelectorAll('.left .chat-avatar')
            .forEach(function (t) {
              i ? t.querySelector('img').setAttribute('src', i) : t.querySelector('img').setAttribute('src', e);
            }),
            window.stop();
        });
      }),
  ...
  function h() {
    var e = new Date().getHours() >= 12 ? 'pm' : 'am',
      t = new Date().getHours() > 12 ? new Date().getHours() % 12 : new Date().getHours(),
  ...
  b();
  var w = document.querySelector('#channel-conversation');
  function E() {
    (channelChatList = w.querySelectorAll('.delete-item')),
  ...
  var S = function (e, t) {
      y++;
      var n = document.getElementById(e).querySelector('.chat-conversation-list');
      null != t &&
      ...
      var a = document.getElementById('chat-list-' + y);
      a.querySelectorAll('.delete-item').forEach(function (e) {
        ...
        a.querySelectorAll('.copy-message').forEach(function (e) {
          e.addEventListener('click', function () {
            var e = a.childNodes[1].firstElementChild.firstElementChild.firstElementChild.firstElementChild.innerText;
        ...
        a.querySelectorAll('.reply-message').forEach(function (e) {
          e.addEventListener('click', function () {
            ...
            var i = e.closest('.chat-list') ? (e.closest('.chat-list').classList.contains('left') ? r : 'You') : r;
            (document.querySelector('.replyCard .replymessage-block .flex-grow-1 .conversation-name').innerText = i),
              (document.querySelector('.replyCard .replymessage-block .flex-grow-1 .mb-0').innerText = a);
          });
        });
    ...
    q = function (e, t) {
      ...
      var r = document.getElementById('chat-list-' + y);
        ...
        r.querySelectorAll('.reply-message').forEach(function (e) {
          e.addEventListener('click', function () {
            var t = e.closest('.ctext-wrap').children[0].children[0].innerText,
              s = document.querySelector('.user-chat-topbar .text-truncate .username').innerHTML;
            document.querySelector('.replyCard .replymessage-block .flex-grow-1 .mb-0').innerText = t;
            var n = e.closest('.chat-list') ? (e.closest('.chat-list').classList.contains('left') ? s : 'You') : s;
            document.querySelector('.replyCard .replymessage-block .flex-grow-1 .conversation-name').innerText = n;
          });
        }),
        r.querySelectorAll('.copy-message').forEach(function (e) {
          e.addEventListener('click', function () {
            r.childNodes[1].children[1].firstElementChild.firstElementChild.getAttribute('id'),
              (isText = r.childNodes[1].children[1].firstElementChild.firstElementChild.innerText),
  ...
  document.getElementById('emoji-btn').addEventListener('click', function () {
    setTimeout(function () {
      ...
      if (e) {
        var t = window.getComputedStyle(e) ? window.getComputedStyle(e).getPropertyValue('left') : '';
